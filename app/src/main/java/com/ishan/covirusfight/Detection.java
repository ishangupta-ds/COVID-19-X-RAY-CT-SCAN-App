...
import com.ishan.covirusfight.vision.AbstractCameraXActivity;
import com.ishan.covirusfight.vision.view.ResultRowView;
...
public class Detection extends AbstractCameraXActivity<Detection.AnalysisResult> {
    ...
    protected AnalysisResult analyzeImage(ImageProxy image, int rotationDegrees) {
        ...
        try {
            ...
            for (int i = 0; i < TOP_K; i++) {
                final int ix = ixs[i];
                topKClassNames[i] = Constants.GTSRB_CLASSES[ix];
            ...
            runOnUiThread(() -> {
                if (!isFinishing()) {
                    showErrorDialog(v -> Detection.this.finish());
                }
            });
            return null;
        }
